import React, { useState, useEffect } from 'react';
import { Modal } from '../../common/Modal';
import { StatusBadge } from '../../common/StatusBadge';
import { IOrder, OrderStatus } from '../../../types';
import { formatCurrency, formatDate } from '../../../utils/formatters';
import { emailTaxInvoiceApi } from '../../../services/orderApi';
import {
  MapPin,
  Phone,
  Mail,
  Truck,
  Send,
} from 'lucide-react';

interface OrderDetailModalProps {
  order: IOrder | null;
  isOpen: boolean;
  onClose: () => void;
  onUpdateStatus: (
    orderId: string,
    status: OrderStatus,
    extraFields?: {
      vehicleNumber?: string;
      driverName?: string;
      driverPhone?: string;
      eWayBillNo?: string;
    }
  ) => Promise<any>;
  onShowToast: (title: string, message?: string, type?: 'success' | 'error' | 'info') => void;
}

const LIFECYCLE_STEPS: { key: OrderStatus; label: string }[] = [
  { key: 'received', label: 'Received' },
  { key: 'confirmed', label: 'Confirmed' },
  { key: 'processing', label: 'Processing' },
  { key: 'dispatched', label: 'Dispatched' },
  { key: 'delivered', label: 'Delivered' },
];

export const OrderDetailModal: React.FC<OrderDetailModalProps> = ({
  order,
  isOpen,
  onClose,
  onUpdateStatus,
  onShowToast,
}) => {
  const [vehicleNumber, setVehicleNumber] = useState(order?.vehicleNumber || '');
  const [driverName, setDriverName] = useState(order?.driverName || '');
  const [driverPhone, setDriverPhone] = useState(order?.driverPhone || '');
  const [eWayBillNo, setEWayBillNo] = useState(order?.eWayBillNo || '');
  const [isUpdatingStatus, setIsUpdatingStatus] = useState(false);
  const [isSendingInvoice, setIsSendingInvoice] = useState(false);

  useEffect(() => {
    if (order) {
      setVehicleNumber(order.vehicleNumber || '');
      setDriverName(order.driverName || '');
      setDriverPhone(order.driverPhone || '');
      setEWayBillNo(order.eWayBillNo || '');
    }
  }, [order]);

  if (!order) return null;

  const handleAdvanceStatus = async (newStatus: OrderStatus) => {
    try {
      setIsUpdatingStatus(true);
      await onUpdateStatus(order._id, newStatus, {
        vehicleNumber: vehicleNumber.trim() || undefined,
        driverName: driverName.trim() || undefined,
        driverPhone: driverPhone.trim() || undefined,
        eWayBillNo: eWayBillNo.trim() || undefined,
      });
      onShowToast(
        'Order Updated',
        `Status successfully transitioned to ${newStatus.toUpperCase()}`,
        'success'
      );
    } catch (err: any) {
      onShowToast('Update Failed', err.message || 'Could not update status', 'error');
    } finally {
      setIsUpdatingStatus(false);
    }
  };

  const handleSaveDispatchInfo = async () => {
    try {
      setIsUpdatingStatus(true);
      await onUpdateStatus(order._id, order.orderStatus, {
        vehicleNumber: vehicleNumber.trim(),
        driverName: driverName.trim(),
        driverPhone: driverPhone.trim(),
        eWayBillNo: eWayBillNo.trim(),
      });
      onShowToast('Dispatch Saved', 'Vehicle and driver details updated', 'success');
    } catch (err: any) {
      onShowToast('Error', err.message || 'Failed to save dispatch details', 'error');
    } finally {
      setIsUpdatingStatus(false);
    }
  };

  const handleEmailInvoice = async () => {
    try {
      setIsSendingInvoice(true);
      const recipient = order.customerEmail || 'client@example.com';
      const res = await emailTaxInvoiceApi({
        orderNumber: order.orderNumber,
        invoiceNumber: `URB/2026/${order.orderNumber.replace(/[^0-9]/g, '')}`,
        recipientEmail: recipient,
        recipientName: order.customerName,
        recipientBusinessName: order.customerName,
        recipientGstin: order.gstin,
        totalAmount: order.totalAmount,
      });
      onShowToast('Tax Invoice Sent', `${res.message} (Tracking: ${res.trackingId})`, 'success');
    } catch (err: any) {
      onShowToast('Invoice Failed', err.message || 'Failed to email invoice', 'error');
    } finally {
      setIsSendingInvoice(false);
    }
  };

  const currentStepIndex = LIFECYCLE_STEPS.findIndex((s) => s.key === order.orderStatus);

  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title={`Order ${order.orderNumber}`}
      subtitle={`Created on ${formatDate(order.createdAt)}`}
      maxWidth="2xl"
    >
      <div className="space-y-6">
        {/* Status Stepper Pipeline */}
        <div className="bg-[#F5F5F7] rounded-2xl p-4 border border-[#E5E5EA]">
          <div className="text-xs font-semibold uppercase tracking-wider text-[#86868B] mb-3">
            Order Lifecycle Pipeline
          </div>
          <div className="grid grid-cols-5 gap-1 text-center">
            {LIFECYCLE_STEPS.map((step, idx) => {
              const isPast = idx < currentStepIndex;
              const isCurrent = idx === currentStepIndex;
              return (
                <button
                  key={step.key}
                  type="button"
                  disabled={isUpdatingStatus}
                  onClick={() => handleAdvanceStatus(step.key)}
                  className={`py-2 px-1 rounded-xl text-xs font-semibold transition-all ${
                    isCurrent
                      ? 'bg-[#007AFF] text-white shadow-xs'
                      : isPast
                      ? 'bg-emerald-100 text-emerald-800'
                      : 'bg-white text-[#86868B] hover:bg-gray-100 border border-[#E5E5EA]'
                  }`}
                  title={`Click to set status to ${step.label}`}
                >
                  <div className="truncate">{step.label}</div>
                  {isCurrent && <div className="text-[10px] opacity-90">Active</div>}
                  {isPast && <div className="text-[10px] text-emerald-600">✓ Done</div>}
                </button>
              );
            })}
          </div>

          {/* Quick status cancel button */}
          {order.orderStatus !== 'cancelled' && order.orderStatus !== 'delivered' && (
            <div className="mt-3 flex justify-end">
              <button
                type="button"
                onClick={() => handleAdvanceStatus('cancelled')}
                disabled={isUpdatingStatus}
                className="text-xs font-medium text-[#FF3B30] hover:underline"
              >
                Mark Order as Cancelled
              </button>
            </div>
          )}
        </div>

        {/* Customer & Site Address Details */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white rounded-xl border border-[#E5E5EA] p-4 space-y-2">
            <span className="text-xs font-semibold uppercase tracking-wider text-[#86868B] block">
              Customer Profile
            </span>
            <div className="text-sm font-bold text-[#1D1D1F]">{order.customerName}</div>
            <div className="flex items-center gap-2 text-xs text-[#1D1D1F] font-mono">
              <Phone className="w-3.5 h-3.5 text-[#86868B]" />
              <a href={`tel:${order.customerPhone}`} className="hover:text-[#007AFF] hover:underline">
                +91 {order.customerPhone}
              </a>
            </div>
            {order.customerEmail && (
              <div className="flex items-center gap-2 text-xs text-[#86868B]">
                <Mail className="w-3.5 h-3.5" />
                <span>{order.customerEmail}</span>
              </div>
            )}
            {order.gstin && (
              <div className="text-xs text-[#86868B] pt-1">
                GSTIN: <span className="font-mono font-semibold text-[#1D1D1F]">{order.gstin}</span>
              </div>
            )}
          </div>

          <div className="bg-white rounded-xl border border-[#E5E5EA] p-4 space-y-2">
            <span className="text-xs font-semibold uppercase tracking-wider text-[#86868B] block">
              Delivery Site Destination
            </span>
            <div className="flex items-start gap-2">
              <MapPin className="w-4 h-4 text-[#007AFF] shrink-0 mt-0.5" />
              <div>
                <div className="text-sm font-semibold text-[#1D1D1F]">
                  {order.siteAddress?.siteName || 'Construction Site'}
                </div>
                <div className="text-xs text-[#86868B] mt-0.5 leading-relaxed">
                  {order.siteAddress?.street}, {order.siteAddress?.city}, {order.siteAddress?.state} -{' '}
                  <span className="font-mono font-semibold">{order.siteAddress?.pincode}</span>
                </div>
                {order.siteAddress?.landmark && (
                  <div className="text-xs text-[#86868B] mt-1 font-medium">
                    Landmark: {order.siteAddress.landmark}
                  </div>
                )}
              </div>
            </div>
            {order.deliveryOtp && (
              <div className="pt-2 border-t border-[#E5E5EA] flex items-center justify-between text-xs">
                <span className="text-[#86868B]">Delivery Verification OTP:</span>
                <span className="font-mono font-bold bg-[#F5F5F7] px-2 py-0.5 rounded border border-[#E5E5EA] text-[#1D1D1F]">
                  {order.deliveryOtp}
                </span>
              </div>
            )}
          </div>
        </div>

        {/* Ordered Material Line Items */}
        <div className="bg-white rounded-xl border border-[#E5E5EA] overflow-hidden">
          <div className="px-4 py-3 border-b border-[#E5E5EA] bg-[#F5F5F7]/40 flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-[#86868B]">
              Ordered Materials ({order.items.length})
            </span>
            <span className="text-xs font-mono text-[#86868B]">HSN & Rate Breakdown</span>
          </div>
          <div className="divide-y divide-[#E5E5EA]">
            {order.items.map((item, i) => (
              <div key={i} className="p-4 flex items-center justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <div className="text-sm font-semibold text-[#1D1D1F]">{item.name}</div>
                  <div className="text-xs text-[#86868B] mt-0.5">
                    Category: {item.category} {item.hsnCode ? `• HSN: ${item.hsnCode}` : ''}
                  </div>
                </div>
                <div className="text-right shrink-0">
                  <div className="text-sm font-semibold font-mono text-[#1D1D1F]">
                    {formatCurrency(item.totalPrice)}
                  </div>
                  <div className="text-xs text-[#86868B] font-mono">
                    {item.quantity} {item.unit} @ {formatCurrency(item.unitPrice)}
                  </div>
                </div>
              </div>
            ))}
          </div>

          {/* Pricing Math Summary */}
          <div className="p-4 bg-[#F5F5F7]/60 border-t border-[#E5E5EA] space-y-1.5 text-xs">
            <div className="flex justify-between text-[#86868B]">
              <span>Subtotal:</span>
              <span className="font-mono font-medium">{formatCurrency(order.subtotal)}</span>
            </div>
            <div className="flex justify-between text-[#86868B]">
              <span>Tax / GST:</span>
              <span className="font-mono font-medium">{formatCurrency(order.taxAmount)}</span>
            </div>
            <div className="flex justify-between text-[#86868B]">
              <span>Freight / Delivery Charges:</span>
              <span className="font-mono font-medium">{formatCurrency(order.deliveryCharges)}</span>
            </div>
            <div className="flex justify-between text-[#86868B]">
              <span>Unloading Charges:</span>
              <span className="font-mono font-medium">{formatCurrency(order.unloadingCharges)}</span>
            </div>
            <div className="pt-2 border-t border-[#E5E5EA] flex justify-between text-sm font-bold text-[#1D1D1F]">
              <span>Grand Total:</span>
              <span className="font-mono text-base text-[#007AFF]">
                {formatCurrency(order.totalAmount)}
              </span>
            </div>
            <div className="flex items-center justify-between pt-1 text-xs">
              <span className="text-[#86868B]">Payment Method: {order.paymentMethod}</span>
              <StatusBadge status={order.paymentStatus} type="payment" />
            </div>
          </div>
        </div>

        {/* Vehicle & Driver Dispatch Assignment (Zero telemetry, pure simple inputs) */}
        <div className="bg-white rounded-xl border border-[#E5E5EA] p-4 space-y-3">
          <div className="flex items-center justify-between">
            <span className="text-xs font-semibold uppercase tracking-wider text-[#86868B] flex items-center gap-1.5">
              <Truck className="w-4 h-4 text-[#1D1D1F]" />
              Vehicle Dispatch Assignment
            </span>
            <span className="text-[11px] text-[#86868B]">Official Driver Contact</span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-[11px] font-semibold text-[#86868B] uppercase mb-1">
                Vehicle Plate Number
              </label>
              <input
                type="text"
                value={vehicleNumber}
                onChange={(e) => setVehicleNumber(e.target.value)}
                placeholder="e.g. TS 07 UA 4821"
                className="w-full bg-[#F5F5F7] text-xs font-mono px-3 py-2 rounded-xl border border-[#E5E5EA] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/20"
              />
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-[#86868B] uppercase mb-1">
                eWay Bill Number
              </label>
              <input
                type="text"
                value={eWayBillNo}
                onChange={(e) => setEWayBillNo(e.target.value)}
                placeholder="e.g. 361088218892"
                className="w-full bg-[#F5F5F7] text-xs font-mono px-3 py-2 rounded-xl border border-[#E5E5EA] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/20"
              />
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-[#86868B] uppercase mb-1">
                Assigned Driver Name
              </label>
              <input
                type="text"
                value={driverName}
                onChange={(e) => setDriverName(e.target.value)}
                placeholder="e.g. Ramesh Yadav"
                className="w-full bg-[#F5F5F7] text-xs px-3 py-2 rounded-xl border border-[#E5E5EA] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/20"
              />
            </div>
            <div>
              <label className="block text-[11px] font-semibold text-[#86868B] uppercase mb-1">
                Driver Phone
              </label>
              <input
                type="tel"
                value={driverPhone}
                onChange={(e) => setDriverPhone(e.target.value)}
                placeholder="e.g. 9876543210"
                className="w-full bg-[#F5F5F7] text-xs font-mono px-3 py-2 rounded-xl border border-[#E5E5EA] focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#007AFF]/20"
              />
            </div>
          </div>

          <div className="flex justify-end pt-2">
            <button
              type="button"
              onClick={handleSaveDispatchInfo}
              disabled={isUpdatingStatus}
              className="py-1.5 px-4 rounded-xl bg-[#1D1D1F] text-white text-xs font-semibold hover:bg-black active:scale-[0.98] transition-all"
            >
              Save Dispatch Details
            </button>
          </div>
        </div>

        {/* Site Delivery Notes if any */}
        {order.notes && (
          <div className="bg-[#F5F5F7] rounded-xl p-3.5 border border-[#E5E5EA] text-xs">
            <span className="font-semibold text-[#1D1D1F] block mb-1">Site Delivery Instructions:</span>
            <p className="text-[#86868B] leading-relaxed">{order.notes}</p>
          </div>
        )}

        {/* Action CTAs */}
        <div className="pt-2 flex flex-col sm:flex-row items-center justify-between gap-3 border-t border-[#E5E5EA]">
          <button
            type="button"
            onClick={handleEmailInvoice}
            disabled={isSendingInvoice}
            className="w-full sm:w-auto flex items-center justify-center gap-2 py-2 px-4 rounded-xl border border-[#E5E5EA] text-xs font-semibold text-[#1D1D1F] hover:bg-[#F5F5F7] active:scale-[0.98] transition-all"
          >
            <Send className="w-3.5 h-3.5 text-[#007AFF]" />
            <span>{isSendingInvoice ? 'Dispatching...' : 'Email Official Tax Invoice'}</span>
          </button>

          <button
            type="button"
            onClick={onClose}
            className="w-full sm:w-auto py-2 px-5 bg-[#007AFF] text-white text-xs font-semibold rounded-xl hover:bg-blue-600 active:scale-[0.98] transition-all"
          >
            Done
          </button>
        </div>
      </div>
    </Modal>
  );
};
